...
export function JsonToTypeScriptConverter() {
    const tTool = useTranslations('Tools.devTools'); const [input, setInput] = useState(SAMPLE);
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-3xl font-black tracking-tight text-foreground flex items-center gap-3">
                        <div className="p-2.5 bg-primary/10 rounded-2xl"><Code2 className="w-7 h-7 text-primary" /></div>{tTool('jSONTOTYPESCRIPT')}</h2>
                    <p className="text-muted-foreground font-medium mt-1">{tTool('convertAnyJSONObjectToTypeScriptInterfacesInstantly')}</p>
                </div>
            </div>

            <div className="flex items-center gap-4">
                <div className="flex-1 space-y-1">
                    <label className="text-xs font-black text-muted-foreground uppercase tracking-widest">{tTool('interfaceName')}</label>
                ...
                <button onClick={() => setInput(SAMPLE)} className="mt-6 flex items-center gap-2 px-4 py-3 bg-muted/30 hover:bg-muted/50 rounded-2xl border-2 border-border font-bold text-sm transition-all">
                    <RefreshCw className="w-4 h-4" />{tTool('sample')}</button>
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-sm font-black text-muted-foreground uppercase tracking-widest">{tTool('jSONInput')}</label>
                    <textarea
                        value={input}
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between">
                        <label className="text-sm font-black text-muted-foreground uppercase tracking-widest">{tTool('typeScriptOutput')}</label>
                    ...
                    <div className="w-full px-5 py-4 bg-muted/20 border-2 border-border rounded-2xl font-mono text-sm min-h-[378px] whitespace-pre-wrap text-foreground/90">
                        {output || <span className="text-muted-foreground">{tTool('typeScriptInterfaceWillAppearHere')}</span>}
                    </div>
                </div>
            </div>
        </div>
