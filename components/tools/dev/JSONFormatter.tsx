...
export function JSONFormatter({ input, setInput, output, setOutput, jsonMode, setJsonMode,
        ...
        <>
            <div className="flex flex-wrap gap-3 items-center">
                <div className="bg-muted p-1 rounded-xl border-2 border-border shadow-sm flex gap-1">
                    ...
                    >
                        <Maximize2 className="h-4 w-4" />{tTool('beautify')}</button>
                    <button
                    ...
                    >
                        <Minimize2 className="h-4 w-4" />{tTool('minify')}</button>
                </div>
                <button
                ...
                >
                    <FileCode className="h-4 w-4" />{tTool('converters')}</button>
            </div>
            ...
            {validationError && (
                <div className="bg-destructive/10 border-2 border-destructive/20 rounded-lg p-4 flex items-start gap-3">
                    ...
                    <div>
                        <h4 className="font-semibold text-destructive mb-1">{tTool('invalidJSON')}</h4>
            ...
            {showConverters && (
                <div className="bg-muted/30 border-2 border-border rounded-xl p-6 space-y-4">
                    <h3 className="font-bold text-lg">{tTool('convertJSONTo')}</h3>
                    <div className="flex gap-3">
                        <button onClick={onConvertToCSV} className="px-6 py-3 bg-primary text-primary-foreground rounded-lg font-bold hover:bg-primary/90 transition-all">{tTool('cSV')}</button>
                        <button onClick={onConvertToXML} className="px-6 py-3 bg-primary text-primary-foreground rounded-lg font-bold hover:bg-primary/90 transition-all">{tTool('xML')}</button>
                    </div>
                </div>
            )}
            ...
            <div className="grid lg:grid-cols-2 gap-6">
                <div className="space-y-3">
                    <div className="flex items-center justify-between">
                        <label className="text-sm font-bold">{tTool('inputJSON')}</label>
