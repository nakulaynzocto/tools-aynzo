...
import { OpenGraphData } from '@/components/types/seo/types';
import { cn } from '@/utils/cn';

interface OpenGraphGeneratorProps {
    og: OpenGraphData;
    setOg: (og: OpenGraphData) => void;
    result: string;
    copied: boolean;
    onCopy: () => void;
}
...
export function OpenGraphGenerator({ og, setOg, result, copied, onCopy }: OpenGraphGeneratorProps) {
            ...
            <div className="p-8 space-y-6 h-full overflow-y-auto no-scrollbar bg-muted/5">
                <div className="space-y-4">
                    <div className="flex items-center gap-2 text-primary border-b border-border pb-4">
                        <Share2 size={16} />
                        <h4 className="text-[10px] font-black uppercase tracking-widest leading-none">{tTool('socialGraph')}</h4>
                    </div>
                    <div className="grid gap-4">
                        <div className="space-y-1.5">
                            <label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pl-1">{tTool('shareTitle')}</label>
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pl-1">{tTool('thumbnailPreviewURL')}</label>
                        ...
                        <div className="space-y-1.5">
                            <label className="text-[9px] font-black uppercase text-muted-foreground tracking-widest pl-1">{tTool('oGDescription')}</label>
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="bg-card rounded-3xl border border-border shadow-xl overflow-hidden">
                            <div className="aspect-[1200/630] bg-muted/30 relative flex items-center justify-center overflow-hidden">
                                {og.image ? (
                                    <img src={og.image} className="w-full h-full object-cover" alt="Preview" />
                                ) : (
                                ...
                                <div className="absolute top-4 left-4 px-3 py-1 bg-black/60 backdrop-blur-md rounded-lg text-[8px] text-white font-black uppercase tracking-widest border border-white/10">{tTool('openGraph')}</div>
                            </div>
                            <div className="p-5 space-y-2 bg-card border-t border-border">
                                ...
                                <h4 className="text-base font-black leading-tight line-clamp-1 text-foreground">
                                    {og.title || 'Compelling Shared Title'}
                                </h4>
                                <p className="text-[11px] text-muted-foreground leading-snug line-clamp-2 font-medium">
                                    {og.description || 'A brief summary that encourages users to click your link on social platforms.'}
                                </p>
                            </div>
                        </div>
                    </div>
