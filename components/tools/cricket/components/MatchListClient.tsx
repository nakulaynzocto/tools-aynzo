...
export default function MatchListClient({ matches }: Props) {
    ...
    if (!isMounted) {
        return <div className="w-full h-96 animate-pulse bg-slate-50 rounded-3xl" />;
    }
            ...
            <div className="bg-white border-t border-slate-100 divide-y divide-slate-100">
                {filteredAll.length === 0 ? (
                    <div className="py-24 text-center bg-white rounded-3xl border border-slate-100 shadow-sm">
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Check back later for updated match lists.</p>
                    </div>
                ) : (
                                    ...
                                    <div className="flex-1 p-5 lg:p-4 flex flex-col justify-center space-y-3 lg:border-r border-slate-100">
                                        ...
                                        <div className="mt-1 flex items-center gap-2">
                                            <div className={`px-3 py-1 rounded text-[8px] font-black uppercase tracking-widest border italic ${
                                                isLive ? 'bg-red-50 text-red-600 border-red-200 animate-pulse shadow-sm' : 'bg-orange-50 text-orange-600 border-orange-200 shadow-sm'
                                            }`}>
