    ...
    return (
        <div className="grid lg:grid-cols-2 gap-10 items-stretch animate-in fade-in zoom-in duration-500">
            <div className="space-y-6">
                <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground">{tCalc('earningsInput')}</h3>
                ...
                <div className="space-y-6 bg-muted/10 p-8 rounded-3xl border-2 border-border/50">
                    ...
                    <div className="space-y-3">
                        <label className="text-sm font-bold text-foreground">{tCalc('workHoursPerWeek')}</label>
                        <input 
                            type="number" 
                            value={hoursPerWeek} 
                    ...
                    <div className="px-5 py-4 bg-primary/10 rounded-2xl border-2 border-primary/20 flex gap-4 items-start">
                        ...
                        <p className="text-[10px] font-bold text-primary/80 leading-relaxed uppercase tracking-wider">{tCalc('calculationsAssume52WeeksPerYearAnd2080StandardUSWorkHoursForFullTimeEmployees')}</p>
                    </div>
                </div>
            </div>

            <div className="flex flex-col gap-6">
                <h3 className="text-xs font-black uppercase tracking-widest text-muted-foreground">{tCalc('wageBreakdown')}</h3>
                ...
                {breakdown ? (
                    ...
                    <div className="bg-muted/10 border-2 border-dashed border-border rounded-3xl flex-1 flex flex-col items-center justify-center text-muted-foreground/30 gap-4 min-h-[400px]">
                        <span className="text-xs font-black uppercase tracking-widest">{tCalc('enterSalaryToSeeWageConversion')}</span>
                    </div>
                )}
            </div>
        </div>
