...
import { Globe, Code, Monitor, Smartphone, Eye, Server, RefreshCw, Copy, ExternalLink, Shield, Settings, Info, Check, Search, CheckCircle2 } from 'lucide-react';
...
export default function WebTools({ type }: WebToolProps) {
    ...
    const [resolution, setResolution] = useState({ w: 375, h: 667, label: 'Mobile' });
                ...
                <div className="p-8">
                    ...
                    {(type === 'screen-resolution-simulator' || type === 'responsive-checker') && (
                        <div className="space-y-10">
                            <div className="flex flex-col xl:flex-row gap-8 items-end">
                                ...
                                <div className="flex p-2 bg-muted rounded-[1.5rem] border-2 border-border gap-2 shadow-inner">
                                    {[
                                        ...
                                        { w: 1440, h: 900, label: 'Desktop', icon: Monitor },
                                    ].map(device => (
                                        <button
                                            key={device.label}
                                            onClick={() => setResolution({ w: device.w, h: device.h, label: device.label })}
                                            className={cn(
                            ...
                            <div className="relative bg-muted/10 border-2 border-border rounded-[3rem] p-12 flex flex-col items-center min-h-[700px] overflow-hidden">
                                ...
                                <div className="mt-12 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] shadow-[0_60px_120px_-20px_rgba(0,0,0,0.4)] border-[12px] border-border rounded-[3.5rem] bg-card p-2 transform hover:scale-[1.01]">
