...
export default function ImageTools({ type }: ImageToolProps) {
    ...
    const isBatchSupported = [
        'image-compressor', 'image-resizer', 'image-enlarger',
        'jpg-to-png', 'png-to-jpg', 'webp-converter',
        'webp-to-jpg', 'webp-to-png', 'jpg-to-webp', 'png-to-webp',
        'svg-to-png', 'png-to-svg', 'jpg-to-svg', 'webp-to-svg'
    ].includes(type);
    ...
    const processSingleFile = async (pf: ProcessedFile): Promise<Blob | string> => {
        return new Promise((resolve, reject) => {
            ...
            img.onload = () => {
                ...
                const qualityValue = ((mime === 'image/png' || mime === 'image/svg+xml') && type !== 'image-compressor')
                    ? undefined
                    : quality / 100;
...
<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
    ...
    useEffect(() => {
            ...
            try {
                ...
                setFiles(prevFiles => prevFiles.map(f => {
                    if (f.id === fileToPreview.id) {
                        let newSize = 0;
                        if (typeof result === 'string') newSize = result.length;
                        else if (result instanceof Blob) newSize = result.size;
        ...
        files, selectedFileId, rotation, flipH, flipV,
        filterValue, color, size, width, height,
        maintainAspectRatio, type, quality
    ]);
    ...
    const imageNavTools = [
        {
            ...
            tools: [
                ...
                { id: 'image-brightness', label: 'Brightness', icon: Zap },
                { id: 'image-contrast', label: 'Contrast', icon: Zap },
                { id: 'blur-image', label: 'Blur', icon: Wand2 },
                { id: 'saturate-image', label: 'Saturate', icon: Wand2 },
                { id: 'hue-rotate-image', label: 'Hue Rotate', icon: RotateCw },
                { id: 'image-opacity', label: 'Opacity', icon: Zap }
            ]
        },
        {
            category: 'CONVERT',
        ...
        },
        {
            ...
            tools: [
                ...
                { id: 'round-corners-image', label: 'Round Corners', icon: CheckCircle2 },
                { id: 'image-border', label: 'Border', icon: Square },
                { id: 'image-shadow', label: 'Shadow', icon: Zap },
                { id: 'pixelate-image', label: 'Pixelate', icon: Grid3X3 }
            ]
        }
    ];
                    ...
                    {files.length === 0 ? (
                        ...
                        >
                            ...
                            {type === 'base64-to-image' && (
                                <div className="mt-6 sm:mt-8 w-full max-w-xl px-4" onClick={e => e.stopPropagation()}>
                                    <textarea
                                        value={base64Input}
                                        onChange={(e) => setBase64Input(e.target.value)}
                                        className="w-full h-20 sm:h-24 p-3 border-2 border-border rounded-xl bg-input font-mono text-xs text-foreground focus:border-accent outline-none resize-none"
                                    ...
                                    <button
                                        ...
                                        className="w-full mt-2 py-2 sm:py-3 bg-primary text-white rounded-xl font-bold transition-all text-xs sm:text-sm"
                                    >
                                        {tActions('convert')}
                                    </button>
                                </div>
                        ...
                        <div className="flex flex-col gap-4 sm:gap-6 overflow-x-hidden">
                            ...
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 px-3 sm:px-6 py-3 sm:py-4 bg-muted/50 rounded-2xl border border-border">
                                <div className="flex items-center gap-2 sm:gap-4">
                                    <div className="flex px-2 sm:px-3 py-1 bg-primary/10 text-primary rounded-lg text-[10px] font-black uppercase tracking-widest border border-primary/20">
                                        {files.length} {t('imagesSelecte', { count: files.length })}
                                    </div>
                                </div>
                                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
                                    <button
                                        onClick={() => document.getElementById('image-input')?.click()}
                                        className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-primary hover:underline flex items-center gap-2"
                                    >
                                        <Upload size={14} /> {tActions('addMore')}
                                    </button>
                                    <button
                                        onClick={() => setFiles([])}
                                        className="text-[10px] sm:text-xs font-black uppercase tracking-widest text-destructive hover:underline"
                                    >
                                        {tActions('clearAll')}
                                    </button>
                                </div>
                            ...
                            {/* Main Content Area */}
                            <div className="flex flex-col lg:grid lg:grid-cols-[1fr,350px] gap-4 lg:gap-6 items-start lg:items-stretch min-w-0">
                                {/* Left side: thumbnails + preview */}
                                <div className="flex flex-col gap-4 sm:gap-6 min-h-0 flex-1 overflow-hidden">
                                    {/* Horizontal File Strip - ONLY THIS ONE */}
                                    <div className="relative group">
                                        {files.length > 3 && (
                                            <>
                                                <button
                                                    ...
                                                    className="absolute left-1 top-1/2 -translate-y-1/2 z-20 bg-background/95 backdrop-blur-sm border border-border rounded-lg p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-all hover:scale-110"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className="absolute right-1 top-1/2 -translate-y-1/2 z-20 bg-background/95 backdrop-blur-sm border border-border rounded-lg p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-all hover:scale-110"
                                                >
                                        ...
                                        <div
                                            id="file-strip"
                                            className="h-24 sm:h-28 lg:h-32 flex-shrink-0 bg-muted/20 p-2 sm:p-3 rounded-2xl border-2 border-border flex items-center gap-2 sm:gap-3 overflow-x-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent hover:scrollbar-thumb-primary/50 scroll-smooth"
                                        >
                                            {files.map((file, index) => (
                                                <div
                                                    ...
                                                    className={cn(
                                                        "h-full aspect-square rounded-xl border-2 transition-all cursor-pointer relative group overflow-hidden flex-shrink-0 min-w-[60px] sm:min-w-[80px] lg:min-w-[100px]",
                                                ...
                                                >
                                                    ...
                                                    {file.status === 'processing' && (
                                                        <div className="absolute inset-0 bg-black/40 backdrop-blur-[1px] flex items-center justify-center">
                                                            <RefreshCw className="w-3 h-3 sm:w-4 sm:h-4 lg:w-5 lg:h-5 animate-spin text-white" />
                                                        </div>
                                                    )}
                                                    {file.status === 'done' && (
                                                        <div className="absolute top-0.5 right-0.5 bg-emerald-500 text-white rounded-full p-0.5 shadow-md">
                                                            <CheckCircle2 size={10} />
                                                        </div>
                                                    )}
                                                    <button
                                                        onClick={(e) => { e.stopPropagation(); removeFile(file.id); }}
                                                        className="absolute -top-1 -left-1 p-0.5 sm:p-1 bg-destructive text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-lg scale-75 hover:scale-100"
                                                    >
                                                        <X size={10} />
                                                    </button>
                                                    <div className="absolute bottom-1 left-1 bg-black/60 text-white text-[8px] sm:text-[10px] px-1 rounded">
                                                        {index + 1}
                                                    </div>
                                                </div>
                                            ))}
                                        ...
                                        {files.length > 5 && (
                                            <div className="absolute -top-2 -right-2 bg-primary text-white text-[10px] px-2 py-1 rounded-full shadow-lg">
                                                +{files.length - 5}
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex-1 bg-gradient-to-br from-muted/20 to-muted/5 p-3 sm:p-4 lg:p-6 rounded-3xl border-2 border-border shadow-inner relative flex flex-col min-h-[200px] sm:min-h-[300px] overflow-hidden">
                                        {(() => {
                                            ...
                                            const comparisonTools = [
                                                'image-compressor', 'image-resizer', 'image-enlarger', 'image-cropper',
                                                'rotate-image', 'flip-image', 'blur-image', 'grayscale-image',
                                                'sepia-converter', 'invert-image', 'image-brightness', 'image-contrast',
                                                'saturate-image', 'hue-rotate-image', 'image-opacity',
                                                'jpg-to-png', 'png-to-jpg', 'webp-converter', 'jpg-to-webp',
                                                'png-to-webp', 'webp-to-jpg', 'webp-to-png', 'svg-to-png', 'png-to-svg'
                                            ];
                                            ...
                                            if (type === 'image-cropper' && selectedFile?.preview) {
                                                return (
                                                    <div className="flex-1 flex flex-col gap-4">
                                                        <div className="grid grid-cols-2 gap-4">
                                                            ...
                                                            <div className="bg-primary/10 border-primary/20 backdrop-blur-sm rounded-xl p-4 border border-primary/30 shadow-sm">
                                                                ...
                                                                <div className="text-2xl font-black text-primary">
                                                                    {completedCrop ? `${Math.round(completedCrop.width)} × ${Math.round(completedCrop.height)}px` : 'Select area'}
                                                                </div>
                                                        ...
                                                        <div className={cn(
                                                            "flex-1 relative rounded-2xl overflow-hidden checkered-bg border-2 border-border shadow-lg min-h-[200px]",
                                                            files.length > 4 ? "max-h-[400px] sm:max-h-[500px] lg:max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent" : "max-h-none"
                                                        )}>
                                                            <div className="absolute inset-0 flex items-center justify-center p-3 sm:p-4">
                                                                <ReactCrop
                                                                    ...
                                                                    className={cn(
                                                                        "max-w-full",
                                                                        files.length > 4 ? "max-h-[250px] sm:max-h-[300px] lg:max-h-[400px]" : "max-h-[400px] sm:max-h-[500px] lg:max-h-[600px]"
                                                                    )}
                                                                >
                                                                    <img
                                                                        ref={imgRef}
                                            ...
                                            if (!showComparison) {
                                                ...
                                                return (
                                                    <div className={cn(
                                                        "flex-1 relative rounded-2xl overflow-hidden checkered-bg border-2 border-border shadow-lg flex items-center justify-center min-h-[200px]",
                                                        files.length > 4 ? "max-h-[400px] sm:max-h-[500px] lg:max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent" : "max-h-none"
                                                    )}>
                                                        {previewUrl ? (
                                                            <div className="relative w-full h-full flex items-center justify-center p-3 sm:p-4">
                                                                <img
                                                                    ...
                                                                    alt="Preview"
                                                                    className={cn(
                                                                        "object-contain drop-shadow-2xl animate-in zoom-in-95 duration-500",
                                                                        files.length > 4 ? "max-w-full max-h-[250px] sm:max-h-[300px] lg:max-h-[400px]" : "max-w-full max-h-[400px] sm:max-h-[500px] lg:max-h-[600px]"
                                                                    )}
                                                                />
                                                            </div>
                                                        ) : (
                                            ...
                                            return (
                                                <div className="flex-1 flex flex-col gap-4">
                                                    <div className="grid grid-cols-2 gap-4">
                                                        ...
                                                        <div className="bg-primary/10 border-primary/20 backdrop-blur-sm rounded-xl p-4 border border-primary/30 shadow-sm">
                                                            ...
                                                            <div className={cn(
                                                                ...
                                                                parseFloat(reductionPercent) > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-muted-foreground"
                                                            )}>
                                                    ...
                                                    <div className={cn(
                                                        "flex-1 relative rounded-2xl overflow-hidden checkered-bg border-2 border-border shadow-lg min-h-[200px]",
                                                        files.length > 4 ? "max-h-[400px] sm:max-h-[500px] lg:max-h-[600px] overflow-y-auto scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent" : "max-h-none overflow-hidden"
                                                    )}>
                                                        <div className={cn(
                                                            "grid grid-cols-2",
                                                            files.length > 4 ? "h-auto min-h-[300px] max-h-[500px]" : "h-auto min-h-[300px]"
                                                        )}>
                                                            <div className="relative flex items-center justify-center p-3 sm:p-4 border-r border-border/30 min-h-[200px]">
                                                                <div className="absolute top-2 sm:top-4 left-2 sm:left-4 bg-black/80 backdrop-blur-sm text-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-[11px] font-black uppercase tracking-widest pointer-events-none z-10">
                                                                    Original
                                                                </div>
                                                                {(type === 'image-resizer' || type === 'image-enlarger') && selectedFile?.preview && (
                                                                    <div className="absolute bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-sm text-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-[11px] font-black uppercase tracking-widest pointer-events-none z-10">
                                                                        {(() => {
                                                                            const img = new Image();
                                                                ...
                                                                <div className="relative w-full h-full flex items-center justify-center">
                                                                    <img
                                                                        ...
                                                                        alt="Original"
                                                                        className={cn(
                                                                            "max-w-full object-contain select-none drop-shadow-lg",
                                                                            files.length > 4 ? "max-h-[200px] sm:max-h-[250px] lg:max-h-[300px]" : "max-h-[350px] sm:max-h-[450px] lg:max-h-[550px]"
                                                                        )}
                                                                        draggable={false}
                                                                    />
                                                                </div>
                                                            ...
                                                            <div className="relative flex items-center justify-center p-3 sm:p-4 min-h-[200px]">
                                                                <div className="absolute top-2 sm:top-4 right-2 sm:right-4 bg-primary/90 backdrop-blur-sm text-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-[11px] font-black uppercase tracking-widest pointer-events-none z-10">
                                                                    {comparisonLabels.right}
                                                                </div>
                                                                {(type === 'image-resizer' || type === 'image-enlarger') && width > 0 && height > 0 && (
                                                                    <div className="absolute bottom-2 sm:bottom-4 right-1/2 translate-x-1/2 bg-black/80 backdrop-blur-sm text-white px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[10px] sm:text-[11px] font-black uppercase tracking-widest pointer-events-none z-10">
                                                                        {width} × {height}px
                                                                    </div>
                                                                )}
                                                                <div className="relative w-full h-full flex items-center justify-center">
                                    ...
                                    <div className="bg-gradient-to-br from-card to-card/50 backdrop-blur-sm p-4 lg:p-6 rounded-3xl border-2 border-border shadow-2xl">
                                                        ...
                                                        <div className="flex items-center justify-between mb-2">
                                                            ...
                                                            <button
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    maintainAspectRatio ? "bg-primary/10 text-primary border border-primary/20" : "bg-muted text-muted-foreground border border-border"
                                                                )}
                                                            >
                                                        ...
                                                        <div className="grid grid-cols-4 gap-2">
                                                            {[0, 90, 180, 270].map((angle) => (
                                                                <button
                                                                    ...
                                                                    className={cn(
                                                                        "py-2 rounded-lg text-[10px] font-black uppercase tracking-wider transition-all border",
                                                                        rotation === angle ? "bg-primary text-primary-foreground border-primary shadow-lg" : "bg-background text-foreground border-border hover:bg-muted"
                                                                    )}
                                                                >
                                                                    {angle}°
                                                                </button>
                                                    ...
                                                    <div className="space-y-4 bg-muted/30 p-4 rounded-xl border border-border/50">
                                                        ...
                                                        <div className="grid grid-cols-2 gap-3">
                                                            <button
                                                                ...
                                                                className={cn(
                                                                    "py-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border flex items-center justify-center gap-2",
                                                                    flipH ? "bg-primary text-primary-foreground border-primary shadow-lg" : "bg-background text-foreground border-border hover:bg-muted"
                                                                )}
                                                            >
                                                            ...
                                                            <button
                                                                ...
                                                                className={cn(
                                                                    "py-4 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all border flex items-center justify-center gap-2",
                                                                    flipV ? "bg-primary text-primary-foreground border-primary shadow-lg" : "bg-background text-foreground border-border hover:bg-muted"
                                                                )}
                                                            >
                                                ...
                                                {type === 'image-cropper' && (
                                                    <div className="space-y-4 bg-muted/30 p-6 rounded-xl border border-border/50">
                                                        <div className="flex items-center justify-between mb-4">
                                                            <label className="text-sm font-bold uppercase tracking-wider text-foreground">Aspect Ratio</label>
                                                            <div className="text-[10px] text-muted-foreground">Choose crop format</div>
                                                        </div>
                                                            ...
                                                            {[
                                                                ...
                                                                return (
                                                                    <button
                                                                        ...
                                                                        className={cn(
                                                                            "py-3 px-4 rounded-lg text-xs sm:text-[10px] font-black uppercase tracking-wider transition-all border",
                                                                            isSelected ? "bg-primary text-primary-foreground border-primary shadow-lg" : "bg-background text-foreground border-border hover:bg-muted"
                                                                        )}
                                                                    >
                                                                        {ratio.label}
                                                                    </button>
                                        ...
                                        <div className="pt-8 mt-auto space-y-4 border-t border-border">
                                            <button
                                                ...
                                                className="w-full py-5 bg-primary text-primary-foreground dark:bg-gradient-to-r dark:from-sky-500 dark:to-blue-600 dark:border-none rounded-2xl font-black text-lg shadow-[0_20px_40px_-15px_rgba(var(--primary-rgb),0.4)] dark:shadow-[0_0_30px_-5px_rgba(14,165,233,0.4)] hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 transition-all flex items-center justify-center gap-3"
                                            >
